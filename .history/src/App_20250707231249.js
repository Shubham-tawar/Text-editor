...
function App() {
  ...
  const toggleMode = () => {
    if (mode === 'light') {
      ...
      document.getElementById('textBox').style.transition = 'background-color 0.5s ease, color 0.5s ease'; // Smooth transition for textarea background and text
      document.getElementsByTagName('button').style.backgroundColor = '#6c757d'; // Change button background color
      document.getElementsByTagName('button').style.color = '#f8f9fa    } else {
      setMode('light');
